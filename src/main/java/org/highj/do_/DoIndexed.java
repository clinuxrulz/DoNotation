...
abstract class DoIndexed<M,S1,S2> {
    ...
    private static <M,S1,S2> __<M,Either<DoWithS1<M,?,S2>,S2>> go2(Monad<M> mMonad, DoWithS1<M,S1,S2> a) {
        return DoIndexedImpl
            .<M,S1,S2>cases()
            .Effect(effect -> mMonad.map(s2 -> Either.<DoWithS1<M,?,S2>,S2>Right(s2), effect.apply(mMonad, a.s1())))
    ...
    public <S3> DoIndexed<M,S1,S3> map(F1<S2,S3> f) {
        return DoIndexedImpl
            .<M,S1,S2>cases()
            .Effect(effect ->
                sequence(
    ...
    private <S3,S4> DoIndexed<M,S1,S3> map_sequence(F1<S2,S3> f, Sequence<M,S1,S4,S2> sequence) {
        return sequence(
            sequence.a(),
